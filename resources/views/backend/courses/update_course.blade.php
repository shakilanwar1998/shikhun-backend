...
<div class="card mb-4 mt-5">
    <h3 class="card-header">Update Course</h3>
                    ...
                    <select class="selectpicker" name="user_id" data-style="btn-default" data-live-search="true" required>
                        @if(count($trainers) > 0)
                            ...
                            @foreach($trainers as $trainer)
                                <option value="{{$trainer->id}}" @if($course->teachers != null) {{ ($course->teachers->id == $trainer->id) ? "selected" : ""}} @endif{{$trainer->first_name}} {{$trainer->last_name}}</option>
                            @endforeach
                        @endif
                    </select>
